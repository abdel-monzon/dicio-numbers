...
public class NiceNumberTest {
    ...
    public void speech() {
        ...
        assertEquals("noventa y ocho y dieciocho diecinueveavos", pf.niceNumber(98.947368421).get());
        assertEquals("menos cinco y seis onceavos", pf.niceNumber(-5.5454545).get());
        assertEquals("siete novenos", pf.niceNumber(7.0 / 9).get());
        assertEquals("menos dos diecisieteavos", pf.niceNumber(-2.0 / 17).get());
    ...
    public void customDenominators() {
        assertEquals("menos cuatro y cuatro décimos", pf.niceNumber(-4.4).denominators(Arrays.asList(2, 3, 4, 6, 7, 8, 9, 10, 11)).get());
        assertEquals("-64 6/12", pf.niceNumber(-64.5).speech(F).denominators(Collections.singletonList(12)).get());
        assertEquals("menos tres y quinientos mil millionésimos", pf.niceNumber(-3.5).denominators(Arrays.asList(1000000, 2000000)).get());
        assertEquals("9 1000000/2000000", pf.niceNumber(9.5).speech(F).denominators(Arrays.asList(2000000, 1000000)).get());
        assertEquals("cero coma ocho", pf.niceNumber(4.0 / 5).denominators(Arrays.asList(2, 3, 4)).get());
    }

    @Test
    public void invalidFraction() {
        assertEquals("uno coma ocho cuatro", pf.niceNumber(1.837).get());

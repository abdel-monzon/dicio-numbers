...
class SpanishDateTimeExtractor internal constructor(
    ...
    fun time(): LocalTime? {
        ...
        return LocalTime.of(number % DateTimeExtractorUtils.HOURS_IN_DAY, 0)
    }
    ...
    fun date(): LocalDate? {
        ...
        var bcad = ts.tryOrSkipDateTimeIgnore(dayOfWeek != null || firstNum != null) { this.bcad() }
        val year = ts.tryOrSkipDateTimeIgnore(
            dayOfWeek != null || firstNum != null && bcad == null
        ) { extractIntegerInRange(0, 999999999) }
        if (year == null) {
            return if (dayOfWeek != null || firstNum != null) result else null
        }
